...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // convert a String object into a JSONObject
            JSONObject reader = new JSONObject(SAMPLE_JSON_RESPONSE);

            JSONArray features = reader.optJSONArray("features");

            // Iterate through the features' elements
            for(int i = 0; i < features.length(); i++) {
                // get magnitude value(double) from thom the "properties" JSONObject
                Double magnitudeDouble = features.optJSONObject(i).optJSONObject("properties").getDouble("mag");

                // make sure the magnitude is represented with only one decimal number
                DecimalFormat formatter = new DecimalFormat("0.0");
                //convert double representation to a string
                String magnitude = formatter.format(magnitudeDouble);

                // get place value from thom the "properties" JSONObject
                String place = features.optJSONObject(i).optJSONObject("properties").getString("place");

                // get time value from thom the "properties" JSONObject
                long time  = features.optJSONObject(i).optJSONObject("properties").getLong("time");

                // the website url for the earthquake
                String url = features.optJSONObject(i).getJSONObject("properties").getString("url");

                // add a new {@link Earthquake} element to the earthquakes list
